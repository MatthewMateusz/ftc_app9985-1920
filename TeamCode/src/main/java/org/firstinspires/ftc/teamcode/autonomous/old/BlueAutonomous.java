...
@Autonomous(name="Blue Autonomous", group="Normal")
public class BlueAutonomous extends Automation {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        switch(location) {
            ...
            case right:
                distanceDrive(direction.right, 0.5 * one_tile, speed_half, 1, true);
                distanceDrive(direction.forward, 0.5*one_tile, speed_half, 1, true);
                setGrabber(grabber.right, grabState.close);
                sleep(300);
                break;
        ...
        //Phase B
        distanceDrive(direction.backward, 0.5 * one_tile, speed_half, 1.5, true);
        RotateArm(true, 2.5);
        sensorDrive(direction.left, speed_half, 10, hardware.sensorDistanceUp, 2.5, false);
        distanceDrive(direction.left, 2 * one_tile, speed_half, 5, true);
        RotateArm(false, 2.5);
        liftArm(true, 1);
        distanceDrive(direction.forward, 0.5 * one_tile, speed_half, 1, true);
